...
def run_codeql_scan(directory, language="cpp", command="make"):
    ...
    init_cmd = ["codeql", "database", "create", f"--language={language}", f"--start-build-command={command}" "db", "--source-root", directory]
    # Replace this with the actual command to run the analysis
    analyze_cmd = ["codeql", "database", "analyze", "db", "--format=csv-sm", "--output=results.csv"]
